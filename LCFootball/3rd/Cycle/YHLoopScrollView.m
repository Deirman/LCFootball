...
@property(nonatomic, assign)NSInteger        countdownShowTime;        //广告展示时间
...
- (void)initialize{
    ...
    /// 初始化scrollView
    self.scrollViewContent = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
...
//    self.scrollViewContent.frame = self.bounds;
    self.scrollViewContent.contentSize = CGSizeMake(self.bounds.size.width * mtkDefaultTotalImgCount, self.bounds.size.height);
    
    /// 循环创建轮播图
    for (NSInteger i = 0; i < mtkDefaultTotalImgCount; i++)
    {
        CGRect frame = CGRectMake(i * self.bounds.size.width, 0, self.bounds.size.width, self.bounds.size.height);
    ...
    _pageCtrlCircle.frame = CGRectMake(0, self.bounds.size.height - 16, self.bounds.size.width, 10);
    ...
    self.imgViewSingle.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
    ...
    self.closeButton = [[UIButton alloc] initWithFrame:CGRectMake(self.bounds.size.width-64, self.bounds.size.height-44, 50, 30)];
    //        [closeButton setTitle:@"关闭" forState:UIControlStateNormal];
    [self.closeButton setTitleColor:[UIColor colorWithHexString:@"#D2D2D2" alpha:1] forState:UIControlStateNormal];
    [self.closeButton.titleLabel setFont:[UIFont systemFontOfSize:14]];
    self.closeButton.tag =1100;

//    [self addSubview:self.closeButton];
    NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:@"关闭"];
    ...
    [str addAttribute:NSForegroundColorAttributeName
                value:[UIColor colorWithHexString:@"#D2D2D2" alpha:1]
                range:strRange];//设置颜色
    [self.closeButton setAttributedTitle:str forState:UIControlStateNormal];

    self.countdownLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width-44, 14, 30, 30)];
    [self.countdownLabel setFont:[UIFont systemFontOfSize:14]];
    [self.countdownLabel setTextColor:[UIColor colorWithHexString:@"#D2D2D2" alpha:1]];
    self.countdownLabel.layer.cornerRadius = 15;
    self.countdownLabel.layer.borderWidth = 1;
    self.countdownLabel.layer.borderColor = [UIColor colorWithHexString:@"#D2D2D2" alpha:1].CGColor;
...
//    [[NSRunLoop currentRunLoop] addTimer:self.timer forMode:NSRunLoopCommonModes];
    ...
    self.countdownShowTime = 3;
    self.countdownTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(countdownTimerAction:) userInfo:nil repeats:YES];
...
#pragma mark 使用默认的轮播速度 还是 自定义轮播速度 ****** 目前采用默认 轮播速度默认
        ...
        if (isDefaultLoopSpeed)
        {
            ...
            [self.scrollViewContent setContentOffset:CGPointMake(self.scrollViewContent.contentOffset.x+self.bounds.size.width, 0)
                                            animated:YES];
        }
        else
        {
...
-(void)countdownTimerAction:(NSTimer*)sender{
    self.countdownShowTime --;
    if (self.countdownShowTime <0) {
        if ([self.delegate respondsToSelector:@selector(closeView)]) {
            [self.delegate closeView];
        }
    }
     [self.countdownLabel setText:[NSString stringWithFormat:@"%lds",self.countdownShowTime]];
}
...
- (void)loopSlowly:(CADisplayLink *)aDisplayLink
{
    ...
    if (self.scrollViewContent.contentOffset.x >= self.bounds.size.width * 2 - spacing)
    {
        CGPoint point = CGPointMake(self.scrollViewContent.contentOffset.x+self.bounds.size.width, 0);
...
-(void)setMArrayImageUrls:(NSMutableArray *)mArrayImageUrls
{
    ...
    if (_mArrayImageUrls.count < 2)
    {
        ...
        if (self.mArrayImageUrls.count > 0)
        {
           ...
           [self.imgViewSingle setBackgroundImage:[UIImage imageNamed:url] forState:UIControlStateNormal];
...
- (void)refreshImage
{
    self.scrollViewContent.contentOffset = CGPointMake(self.bounds.size.width, 0);
    ...
    for (NSInteger i = 0; i < count; i++)
    {
        ...
        [weakBtnImageView setBackgroundImage:[UIImage imageNamed:url] forState:UIControlStateNormal];
...
//        }];
    ...
    if (self.mArrayImageUrls.count == 2)
    {
        ...
        weakBtnImageView.tag = [self indexWithUrl:tempUrl];
         [weakBtnImageView setBackgroundImage:[UIImage imageNamed:tempUrl] forState:UIControlStateNormal];
//        [weakBtnImageView sd_setBackgroundImageWithURL:[NSURL URLWithString:[tempUrl substringToIndex:tempUrl.length-1]]
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (scrollView == _scrollViewContent) { // 如果此时滑动的是_scrollView,避免影响tableView的滑动
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    if (string.length != 1)
    {
        [self.scrollViewContent setContentOffset:CGPointMake(self.bounds.size.width, 0)
                                        animated:YES];
    }
}
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    // 当前控件上的点转换到chatView上
    CGPoint chatP = [self convertPoint:point toView:self.closeButton];

    // 判断下点在不在chatView上
    if ([self.closeButton pointInside:chatP withEvent:event]) {
